...
#+begin_src sh
sudo apt install qemu-user-binfmt equivs mmdebstrap dpkg-dev
#+end_src
...
$ ./base-os-build.sh                                                    \
    ...
    --release trixie                                                    \
    --aptrepos https://fatty.secretsauce.net/debian/sample-project/     \
    --aptopt 'Acquire::https::fatty.secretsauce.net::Verify-Peer=false' \
    --deps                                                              \
    sample-project.equivs
#+end_example
...
dependent packages are defined in the [[file:sample-project.equivs][=sample-project.equivs= file]]. The relevant
data in each meta-package
...
$ dpkg -I sample-project-deps-robot-arm64_0.01_amd64.deb
...
I push those meta-packages to the local APT repository (configured in
=/etc/dput.cf=) that everybody uses to get custom packages for this project:

#+begin_example
$ dput sample-project-repo *.changes
#+end_example
...
We will build only those tarballs in =--flavors=.
...
The tooling in this package is accessed through [[file:base-os-build.sh][=base-os-build.sh=]]. You can
=./base-os-build.sh --deps ...= to build the meta-packages, push them to the APT
server and =./base-os-build.sh --tarballs ...= to build OS tarballs with the
meta-package preinstalled (see the sample invocation above). If both =--deps
--tarballs= are given, we will build the metapackage and then use the local
metapackage file for the tarball; this allows running without a custom APT
server at all.

Metapackage definitions are defined in a file filtered by the =base-os-build.sh=
and then processed by the =equivs= tool to create the package. The OS tarballs
are made with =mmdebstrap=.
